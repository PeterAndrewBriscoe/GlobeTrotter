        ...
        <form id="option-form" onSubmit={handleSubmit}>
            <div id="options-grid">
                <div>
                    ...
                    {art? <>{(Math.round(artValue * 100) / 100).toFixed(1)} <input className="slider" type="range" name="artValue" min="5" max="10" value={artValue} step="0.2" onChange={handleSlide}></input></> : <></>}
                </div>
                <div>
                    <input type="checkbox" onChange={handleCheck} value="1" name="beaches"/>Beaches
                    <i className="fas fa-umbrella-beach"></i>
                    {beaches? <>{(Math.round(beachesValue * 100) / 100).toFixed(1)} <input className="slider" type="range" name="beachesValue" min="5" max="12" value={beachesValue} step="0.2" onChange={handleSlide}></input></> : <></>}
                </div>
                <div>
                    <input type="checkbox" onChange={handleCheck} value="2" name="cuisine"/>Cuisine
                    <i className="fas fa-utensils"></i>
                    {cuisine? <>{(Math.round(cuisineValue * 100) / 100).toFixed(1)} <input className="slider" type="range" name="cuisineValue" min="5" max="10" value={cuisineValue} step="0.2" onChange={handleSlide}></input></> : <></>}
                </div>
                <div>
                    <input type="checkbox" onChange={handleCheck} value="3" name="golf"/>Golf
                    <i className="fas fa-golf-ball"></i>
                    {golf? <>{(Math.round(golfValue * 100) / 100).toFixed(1)} <input className="slider" type="range" name="golfValue" min="5" max="10" value={golfValue} step="0.2" onChange={handleSlide}></input></> : <></>}
                </div>
                <div>
                    <input type="checkbox" onChange={handleCheck} value="3" name="museums"/>Museums
                    <i className="fas fa-landmark"></i>
                    {museums? <>{(Math.round(museumsValue * 100) / 100).toFixed(1)} <input className="slider" type="range" name="museumsValue" min="5" max="10" value={museumsValue} step="0.2" onChange={handleSlide}></input></> : <></>}
                </div>
                <div>
                    <input type="checkbox" onChange={handleCheck} value="3" name="skiing"/>Skiing
                    <i className="fas fa-skiing"></i>
                    {skiing? <>{(Math.round(skiingValue * 100) / 100).toFixed(1)} <input className="slider" type="range" name="skiingValue" min="2" max="4.6" value={skiingValue} step="0.2" onChange={handleSlide}></input></> : <></>}
                </div>
                <div>
                    <input type="checkbox" onChange={handleCheck} value="3" name="hiking"/>Hiking
                    <i className="fas fa-hiking"></i>
                    {hiking? <>{(Math.round(hikingValue * 100) / 100).toFixed(1)} <input className="slider" type="range" name="hikingValue" min="5" max="13" value={hikingValue} step="0.2" onChange={handleSlide}></input></> : <></>}
                </div>
                <div>
